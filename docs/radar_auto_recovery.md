...
화면의 최상위 `LIVE`/`DEGRADED` 표시는 **현재 캡처 구간**의 상태만 나타낸다.
진단 카운터는 실행 기간 전체의 누적값이며, mission의 최종 health도 누적 결과로
남는다. 따라서 과거 구간의 진단 이력이나 최종 health를 현재 구간의
`LIVE`/`DEGRADED` 표시와 혼동하지 않는다. 새 프레임이 계속 들어오는 동안 한
프레임만 빠지는 경우에는 supervisor 복구를 시작하지 않는다.
